<div class="max-w-xl mx-auto mt-10 bg-white p-6 rounded-2xl shadow-lg space-y-6">
    ...
    @if (session()->has('error'))
        <p class="text-red-600 font-medium bg-red-100 p-2 rounded">
            {{ session('error') }}
        </p>
    @endif
    ...
    @forelse ($products as $product)
        <div class="p-4 mb-4 border border-gray-200 rounded-lg shadow-sm bg-gray-50">
            <div class="flex justify-between items-start">
                ...
                <button wire:click="deleteProduct({{ $product->id }})" class="bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700 text-sm">
                    Remove
                </button>
            </div>
        </div>
